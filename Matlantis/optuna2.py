...
def get_all_molecules():
    molecules = []
    for fname in ["2-ketone.cif", "3-ketone.cif", "4-ketone.cif", "5-ketone.cif","2-6ketone.cif","3-6ketone.cif"]:
...
def objective(trial):
    ...
    add_adsorbate(slab, mol, height=0.0, position=xy_position)

    mol_ads = slab[-len(mol):]
    
    mol_ads = adjust_adsorption_height(slab, mol_ads, hight=z_hig)
    slab[-len(mol):].positions = mol_ads.positions
